...
vector<MarketModel*> parseLinkModel(string name) {
    ...
    while(!inputFile.eof()){
        ...
        //Smart meters are organized and grouped into clusters of 20 nodes forming local area networks
...
int main (int argc, char *argv[])
{
    ...
    chelper.SetChannelAttribute ("DataRate", DataRateValue (10000000));
    chelper.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (int_delay)));
    ...
    for (size_t netIndex=0; netIndex<networks.size(); ++netIndex)
    {
        ...
        //phelper3.SetChannelAttribute ("Delay", StringValue ("3000000ms"));

        phelper3.SetDeviceAttribute ("DataRate", StringValue ("4Mbps"));
        phelper3.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (int_delay)));
